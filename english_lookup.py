...
_SINGLE_NUMERIC_RE = re.compile(r"^([0-9]+)(?:[.!?])?$")
_NUMERIC_SEQUENCE_RE = re.compile(r"^[0-9]+(?:[\s,，、]+[0-9]+)*(?:[.!?。！？])?$")
...
def _validate_text(payload):
    ...
    numeric_query = _NUMERIC_SEQUENCE_RE.fullmatch(normalize_query(text))
...
def _dictionary_key(text):
    ...
    match = _SINGLE_NUMERIC_RE.fullmatch(key)
